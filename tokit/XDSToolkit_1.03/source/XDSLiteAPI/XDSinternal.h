...
public:
	...
	{
		CTrieIterator it(*this);
		for(; !it.AtEnd(); ++it)
		{
			struct xdsRecord_t *pRecord = (struct xdsRecord_t*)it.GetData();
			printf("%04x: %04x %s\n", pRecord->iClass, pRecord->iRecordId, pRecord->szName);
		}
	}

	void PrintKey(const char *szPrefix, void *data)
	{
		printf("%s", szPrefix);
		struct xdsRecord_t *pRecord = (struct xdsRecord_t*)data;
		printf("%04x: %04x %s\n", pRecord->iClass, pRecord->iRecordId, pRecord->szName);
	}
#endif
};
...
public:
	virtual CIdTrie::~CIdTrie()
	{
		CTrieIterator it(*this);
		for(; !it.AtEnd(); ++it)
		{
			struct xdsType_t *data = (struct xdsType_t*)it.GetData();

			this->Del((unsigned char*)(data->szName), strlen(data->szName));
			if(data->iClass == XDS_RECORD_TYPEDEF)
			{
				if(data->iType == XDS_META_TYPE_STRUCTURE)
				{
					// free all fields
					for(int ii = 0; ii < data->var.struct_t.iFieldCount; ++ii)
					{
						XDS_FREE((void*)(data->var.struct_t.pFields[ii].szName));
					}
					XDS_FREE(data->var.struct_t.pFields);
				}
				else if(data->iType == XDS_META_TYPE_ENUMERATION)
				{
					// free all enums
					for(int ii = 0; ii < data->var.enum_t.iEnumCount; ++ii)
					{
						XDS_FREE((void*)(data->var.enum_t.pEnums[ii].szName));
					}
					XDS_FREE(data->var.enum_t.pEnums);
				}
			}

			XDS_FREE((void*)(data->szName));
			XDS_FREE(data);
		}
	}
