...
public:
	...
	const char *GetText(void) const { return m_szText ? m_szText : ""; };
...
static int xmlTagName(struct xdsHandle *hXds)
{
	...
	// scan for tag end
	int ii = 1;
	for(; ch = hXds->xBuf.Lookahead(ii), (isalnum(ch) || ch == ':' || ch == '_' || ch == '-'); ++ii)
		;
...
static bool xmlReadDocname(struct xdsHandle *hXds)
{
	...
	{
		if(strcmp(hXds->szStreamName, szDocName))
		{
            XDS_ERROR("XML document type does not match XDS stream name");
			XDS_FREE((void*)szDocName);
			return false;
		}
...
static CxdsXmlNode *xmlParseRecord(struct xdsHandle *hXds)
{
	...
	for(;;)
	{
		...
		{
			XDS_ERROR("Badly formed XML data");
			delete pTopNode;
			return NULL;
		}
	}
...
static int xdsXmlDecodeString(CxdsXmlNode *pXmlNode, u16 iType, struct xdsType_t *pType, u8 *pElemData)
{
	...
	memcpy(pElemData, pXmlNode->GetText(), iLen);
	pElemData[iLen] = '\0';
...
//		struct xdsType_t *pBaseType = (struct xdsType_t *)hXds->pTypeDb->Find((unsigned char*)&pType->var.array_t.iBaseType, sizeof(pType->var.array_t.iBaseType)); 
		...
		int ii = 0;
		for(; ii < pXmlNode->NumChildren(); ++ii)
		{
...
//				sizeof(pField->iType));
			...
			if(pField->iArraySize > 1)
			{
				...
				else
				{
					...
					{
						int iSize = xdsXmlDecodeElement(hXds, pXmlField->GetChild(ii), pField->iType, pBaseType, pElemData+iOffset);
						if(iSize < 0)
							return -1;
...
static bool xdsXmlProcRecord(struct xdsHandle *hXds, CxdsXmlNode *pXmlRecord)
{
	...
	if(!pRecord || pRecord->iClass != XDS_RECORD_RECORDDEF)
	{
        ...
        hXds->pTypeDbByName->ListContents();
...
bool xdsXmlReadRecord(struct xdsHandle *hXds)
{
	...
	bool bSuccess = xdsXmlProcRecord(hXds, pNode);
	delete pNode;
	return bSuccess;
}
