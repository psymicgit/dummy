...
struct testst2_t { u32 test1; u8 test;};
struct testst3_t { u16 test1; u16 test2; u8 test;};
...
//	struct testst2_t testst2;
//	struct testst3_t testst3;
	if((XDS_CONFIG_ALIGN != ((u8*)&testst.test1 - (u8*)&testst)) ||  // field alignment is correct?
		((sizeof(struct testst2_t)%XDS_CONFIG_ALIGN) != 0) ||        // structure alignments are correct?
		((sizeof(struct testst3_t)%XDS_CONFIG_ALIGN) != 0))
		XDS_ERROR("Badly configured structure alignment size");
#else
	testst3_t testst3;
	if((sizeof(testst) != sizeof(testst.test1)*2) || (sizeof(testst3) != sizeof(testst3.test1)*3))
		XDS_ERROR("Badly configured structure alignment size");
#endif
	...
	struct xdsHandle *hXds = new struct xdsHandle;		// must do 'new' since xdsHandle contains class instances...
