...
//		break;
	...
	{
		(void)pDSD->ReadShort();
		const char *szStreamName = pDSD->ReadTag();
		if(hXds->szStreamName != NULL)
		{
			if(strcmp(hXds->szStreamName, szStreamName))
			{
				XDS_ERROR("Mismatched XDS stream name");
				XDS_FREE((void*)szStreamName);
				return false;
			}
			XDS_FREE((void*)hXds->szStreamName);
		}
		hXds->szStreamName = szStreamName;
		break;
	}
...
{
	if(iLengthSize != 1 && iLengthSize != 2 && iLengthSize != 4)
	{
		XDS_ERROR("Invalid record length size in record definition");
		return 0;
	}
...
//	struct xdsType_t *pType = (struct xdsType_t*)hXds->pTypeDb->Find((unsigned char*)&iStructType, sizeof(iStructType));
	...
	// keep track of most coarsely aligned field size
	if(elem.iNodeSize > pType->iNodeSizeLength)
		pType->iNodeSizeLength = (unsigned char)elem.iNodeSize;
...
#if (XDS_CONFIG_ALIGN != 0)
	pType->iNodeSize = alignOffset(pType->iNodeSize, XDS_CONFIG_ALIGN);       // align to fixed structure alignment
#else
	pType->iNodeSize = alignOffset(pType->iNodeSize, pType->iNodeSizeLength); // align to coarsest field alignment
	pType->iNodeSizeLength = 0;
#endif
