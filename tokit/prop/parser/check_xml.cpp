///<------------------------------------------------------------------------------
//< @file:   prop\parser\check_xml.cpp
//< @author: hongkunan
//< @date:   2014年4月6日 13:23:34
//< @brief:  
...
namespace checkutil
{
	...
	bool check(string &xlsx, string &datadir)
	{
		if(false == fileuitl::exist(xlsx))
		{
			ECHO_ERR("错误: %s 路径不存在", xlsx.c_str());
			return false;
		}
		...
		propunit_t unit;
		errvec_t errvec;
		parseutil::parse_excel(xlsx, unit, errvec);
