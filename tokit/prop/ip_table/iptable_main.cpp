...
void write_to_file(ipvec_t &ipvec)
{
    ...
    */
    ...
    ofstream o("ip_table.sql");

    char buf[64] = {0};

    o << "truncate table ip;" << endl;
    for(ipvec_t::iterator itr = ipvec.begin(); itr != ipvec.end(); ++itr)
    {
        ip_t &ip = *itr;

        o<< "replace into ip values(";

        string str_ip_start = "'";
        str_ip_start += ip.start;
        str_ip_start += "',";

        sprintf_s(buf, sizeof(buf), "%-19s", str_ip_start.c_str());
        o << buf << ' ';
        ...
        sprintf_s(buf, sizeof(buf), "%-19s", str_ip_end.c_str());
        o << buf << ' ';
        o << "'" << ip.addr << "');" << endl;
    }
}
