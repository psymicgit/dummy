...
namespace SGDP 
{
     ...
     */
	const SSGDPVersion	SDLOGGER_VERSION    = { 3, 2, 0, 0 };
...
#define LOGLV_CRITICAL  (0x0001<< 3)
	...
	class ISDLogger
	{
		public:
             ...
             */
			virtual bool SDAPI LogText(const TCHAR* pszLog) = 0;
             ...
             */
			virtual bool SDAPI LogBinary(const UINT8* pLog, UINT32 dwLen) = 0;
	};
	...
	class ISDFileLogger : public ISDLogger
	{
		public:
			...
			*/
			virtual bool SDAPI Init(const TCHAR* pszDir, const TCHAR* pszFileName, INT32 nMode) = 0;
	...
	class ISDUdpLogger :  public ISDLogger
	{
		public:
			...
			* @return operation result
			*/
			virtual bool SDAPI Init(const TCHAR* pszIp, const UINT16 wPort, UINT8 byServerType, const TCHAR* pszErrorfile = NULL) = 0;
	...
	{
		public:
			virtual BOOL SDAPI Init(const TCHAR* szQueuePath, INT32 sendtype) = 0;
			virtual VOID SDAPI Release() = 0;
	...
	* inc sdlogger.h
	*/
	typedef ISDUdpLogger* (SDAPI *PFN_SDCreateUdpLogger)(const SSGDPVersion* pstVersion);
	...
	* @return	he pointer of ISDUdpLogger if succeeded£¬NULL for failed
	  */
	ISDUdpLogger* SDAPI SDCreateUdpLogger(const SSGDPVersion* pstVersion);

	/**
	* @brief typedef SDCreateTcpLogger function type inc sdlogger.h
    */
	typedef ISDTcpLogger* (SDAPI *PFN_SDCreateTcpLogger)(const SSGDPVersion* pstVersion);
	...
	* @return	the pointer of ISDTcpLogger if succeeded£¬NULL for failed
	  */
	ISDTcpLogger* SDAPI SDCreateTcpLogger(const SSGDPVersion* pstVersion);

	/**
	* @brief typedef SDCreateFileLogger function type inc sdlogger.h
	*/
	typedef ISDFileLogger* (SDAPI *PFN_SDCreateFileLogger)(const SSGDPVersion* pstVersion);
    ...
    */
	ISDFileLogger* SDAPI SDCreateFileLogger(const SSGDPVersion* pstVersion);
	...
	*/
	ISDMsmqLogger* SDAPI SDCreateMsmqLogger(const SSGDPVersion* pstVersion);

	/**
	* @brief typedef SDCreateMsmqLogger function type inc sdlogger.h
	*/
	typedef ISDMsmqLogger* (SDAPI *PFN_SDCreateMsmqLogger)(const SSGDPVersion* pstVersion);
#else 
	/**
	* @brief for sdcommon.def
	*/
	void * SDAPI SDCreateMsmqLogger(const SSGDPVersion* pstVersion);
	...
	class CSDLogger
	{
		public:
             ...
             */
			void SDAPI SetLogger(ISDLogger* pLogger);

            /**
             * @brief log by LOGLV_CRITICAL
             * @param format : same as printf
             */
			void SDAPI Critical(const TCHAR *format, ...);

            /**
             * @brief log by LOGLV_INFO
             * @param format : same as printf
             */
			void SDAPI Info(const TCHAR *format, ...);

            /**
             * @brief log by LOGLV_WARN
             * @param format : same as printf
             */
			void SDAPI Warn(const TCHAR *format, ...);

            /**
             * @brief log by LOGLV_DEBUG
             * @param format : same as printf
             */
			void SDAPI Debug(const TCHAR *format, ...);
