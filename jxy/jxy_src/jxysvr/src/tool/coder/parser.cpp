...
bool CParser::Parse(LPCTSTR pszXml)
{
	...
	{
		m_strErrMsg.Format("Can not Load XML File %s", pszXml);
		return false;
	}
...
bool CParser::_ParseTypes()
{
	...
	if (pTypesElement)
	{
		...
		while (pTypeElement)
		{
			if (false == _ParseAType(pTypeElement, strComment))
			{
				return false ;
			}
...
bool CParser::_ParseAType(TiXmlElement* pTypeElement, CString strComment)
{
