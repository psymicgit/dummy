...
public:
	...
	bool Compile(LPCTSTR pszFile, BOOL bCliEnDe, BOOL bSaveEnDe = FALSE);
...
protected:
	bool _Parse(LPCTSTR pszFile, BOOL bCliEnDe);
	bool _Check();
	bool _CheckName();
	bool _CheckTypes();
	...
	bool _WriteCliIndex();
	bool _WriteIndex(CParser* poParser, FILE* pfStructIndexFile, FILE* pfUnionIndexFile);
