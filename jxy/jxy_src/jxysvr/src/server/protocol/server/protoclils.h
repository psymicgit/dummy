...
typedef struct tagPKT_CLILS_LOGIN_ACK{
	...
	TCHAR            aszUserName[USERNAME_LEN]; //用户名
	UINT16           wZoneID; //区ID
	TCHAR            aszZoneName[ZONENAME_LEN]; //区名称
	UINT32           dwPlayerID; //玩家ID，老用户登录有效
	UINT16           wCareerID; //职业，创建新玩家(PlayerID为0)有效
	DT_EXT_FUNC_PARAM_LST stExtFuncParamLst; //扩展功能参数列表
}PKT_CLILS_LOGIN_ACK;
...
typedef struct tagPKT_CLILS_LOGIN_EX_REQ{
	...
	UINT16           wCareerID; //职业类型
	UINT8            byAuthType; //认证类型
	TCHAR            aszAuthKey[AUTH_KEY]; //认证字符串
	DT_EXT_FUNC_PARAM_LST stExtFuncParamLst; //扩展功能参数列表
...
typedef struct tagDT_EXPAND_FUNC_PARAM{
	UINT8            byExpandType; //扩展类型
	UINT8            byExpandLen; //扩展长度
	UINT8            abyExpandData[MAX_EXPAND_DATA_LEN]; //参数3
}DT_EXPAND_FUNC_PARAM;

typedef struct tagDT_EXPAND_FUNC_PARAM_LST{
	UINT8            byExpandNum; //扩展功能数
	DT_EXPAND_FUNC_PARAM astExpandFuncParaInfo[MAX_EXPAND_FUN_NUM]; //扩展功能信息
...
typedef struct tagPKT_CLILS_GET_USER_EXPAND_ACK{
	...
	UINT16           wErrCode; //错误码
	DT_EXPAND_FUNC_PARAM_LST stExpandFuncParaLst; //用户扩展信息
