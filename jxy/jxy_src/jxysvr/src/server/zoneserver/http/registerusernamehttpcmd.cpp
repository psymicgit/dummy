...
CRegisterUserNameHttpCMD::CRegisterUserNameHttpCMD()
{
    memset(&stReq, 0x00, sizeof(stReq));
    memset(&stAck, 0x00, sizeof(stAck));
    m_strUrl = ((CZSApplication*)SDGetApp())->GetZSConfig()->GetRegist();
}
...
BOOL  CRegisterUserNameHttpCMD::Init()
{
    CNGString strTemp;
    char *efc = NULL;
...
BOOL CRegisterUserNameHttpCMD::OnExecuted()
{
    if (FALSE == m_bError)
    {
        stAck.wErrCode = ERR_REGISTER_USERNAME::ID_SUCCESS;
        ResponseMsg(CLIZS_REGISTER_USERNAME_ACK, (CHAR*)(&stAck));
    }
    else
    {
        ProcessResult();
...
VOID CRegisterUserNameHttpCMD::SetUserData(VOID* pUserData, UINT32 dwLen, UINT32 dwTransID)
{
...
VOID CRegisterUserNameHttpCMD::ProcessResult()
{
