...
VOID CBindSmsCMD::OnExecuted(void)
{
...
//数据执行
VOID CBindSmsCMD::OnExecute(void)
{
    PKT_CLILS_BIND_MOBILE_REQ *pReq = (PKT_CLILS_BIND_MOBILE_REQ*)m_pUserData;
	...
	CHAR		szMsg[256] = {0};
	sprintf(szMsg, "手机绑定验证码: %s - 《囧西游》", abyVerCode);
    ...
    if (SDGBK2UTF8(RESULTSTR) == m_strNetData)
    {
        ...
        CSmsUserMgr::Instance()->CreateUser(pReq->dwUserID, (CHAR*)pReq->abyMObile, em_BIND, abyVerCode);
    }
    else
    {
