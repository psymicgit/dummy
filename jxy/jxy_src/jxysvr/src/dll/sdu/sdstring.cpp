...
#define SDPAGE                          4096
	...
	std::string ReplaceStr(std::string strSrc, std::string strOld, std::string strNew)
	{	
		for(std::string::size_type nPos = 0; nPos != string::npos; nPos += strNew.length())
		{		
			nPos = strSrc.find(strOld.c_str(), nPos);
			if(nPos != string::npos)
			{
				strSrc.replace(nPos, strOld.length(), strNew);
			}
			else
			{
				break;
	...
	std::string ReplaceStrLoop(std::string strSrc, std::string strOld, std::string strNew)
	{
		while(1)
		{
			std::string::size_type nPos = 0;
			nPos = strSrc.find(strOld.c_str());
			if(nPos != string::npos)
			{
				strSrc.replace(nPos, strOld.length(), strNew);
			}
	...
	std::string  SDGBK2UTF8(const CHAR *src)
	{
		CHAR szDesc [SDPAGE*10] = {0};
		size_t nSize = SDPAGE*10;
		SDIconvCovert(UTF8_CHARSET, "GBK", (CHAR*)src,SDStrlen(src),(CHAR*)szDesc,nSize);
		std::string strResult = szDesc;
		return strResult;
	}
