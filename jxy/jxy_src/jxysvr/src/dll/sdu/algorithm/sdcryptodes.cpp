...
#define SWAP(a,b) { UINT32 t = a; a = b; b = t; t = 0; }
	...
	static VOID DESSetKeySub( UINT32 SK[32], UCHAR key[8] )
	{
		INT32  i;
		UINT32 X, Y, T;
		...
		UCHAR *pacOutput )
	{
		...
		if( nMode == DES_ENCRYPT )
		{
			while ( nLength > 0 )
			{
				for ( i = 0; i < 8; i++ )
				{
					pacOutput[i] = (UCHAR)( pacInput[i] ^ pIV->acBlock[i] );
				}
		...
		else /* DES_DECRYPT */
		{
			while ( nLength > 0 )
			{
				...
				for ( i = 0; i < 8; i++ )
				{
					pacOutput[i] = (UCHAR)( pacOutput[i] ^ pIV->acBlock[i] );
				}
	...
	static VOID DES3Setkey( UINT32 esk[96], UINT32 dsk[96], UINT8  key[24] )
	{
		int i;
	...
	//
	static const UCHAR des_test_keys[24] =
	{
	...
	static const UCHAR des_test_iv[8] =
	{
		0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF,
	};

	static const UCHAR des_test_buf[8] =
	{
		0x4E, 0x6F, 0x77, 0x20, 0x69, 0x73, 0x20, 0x74
	};

	static const UCHAR des_test_ecb_dec[3][8] =
	{
	...
	static const UCHAR des_test_ecb_enc[3][8] =
	{
	...
	static const UCHAR des_test_cbc_dec[3][8] =
	{
	...
	static const UCHAR des_test_cbc_enc[3][8] =
	{
	...
	static const UCHAR des3_test_keys[24] =
	{
		0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
		0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x01,
		0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x01, 0x23
	...
	static const UCHAR des3_test_iv[8] =
	{
		0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF,
	};

	static const UCHAR des3_test_buf[8] =
	{
		0x4E, 0x6F, 0x77, 0x20, 0x69, 0x73, 0x20, 0x74
	};
	...
	INT32 DESSelfTest( INT32 nVerbose )
	{
		...
		for ( i = 0; i < 2; i++ )
		{
			...
			{
			case 0:
				DESSetDecKey( &ctx, (UCHAR *) des_test_keys );
				break;

			case 1:
				DESSetEncKey( &ctx, (UCHAR *) des_test_keys );
				break;
