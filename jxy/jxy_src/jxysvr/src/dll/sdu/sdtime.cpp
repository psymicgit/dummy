...
INT64 CSDDateTime::operator - (const CSDDateTime & datetime)
{
    ...
    tm_b = mktime(&t_tm);
    s_oMutex.Unlock();
    return (INT64)difftime(tm_a, tm_b);
}
...
INT64 CSDDateTime::DiffMinute(const CSDDateTime & datetime)
{
    ...
    tm_b = mktime(&t_tm);
    s_oMutex.Unlock();
    return ((INT64)difftime(tm_a, tm_b)) / 60;
}
...
INT64 CSDDateTime::DiffHour(const CSDDateTime & datetime)
{
    ...
    tm_b = mktime(&t_tm);
    s_oMutex.Unlock();
    return ((INT64)difftime(tm_a, tm_b)) / (60 * 60);
}
...
INT64 CSDDateTime::DiffDay(const CSDDateTime & datetime)
{
    ...
    tm_b = mktime(&t_tm);
    s_oMutex.Unlock();
    return ((INT64)difftime(tm_a, tm_b)) / (60 * 60 * 24);
}

INT32 CSDDateTime::DiffDateDay(const CSDDateTime &datetime)
{
    CSDDateTime oMy = *this;
    oMy.SetDateTime(GetYear(), GetMonth(), GetDay(), 0, 0, 0);
    CSDDateTime oDst = datetime;
    oDst.SetDateTime(datetime.GetYear(), datetime.GetMonth(), datetime.GetDay(), 0, 0, 0);
    tm t_my = oMy.m_time;
    tm t_dst = oDst.m_time;
    ...
    tm_b = mktime(&t_dst);
    s_oMutex.Unlock();
    FLOAT fGap = ((FLOAT)difftime(tm_a, tm_b)) / (60 * 60 * 24);
    FLOAT fLess = fGap - INT32(fGap);
    if((fabs(fLess) > 1e-6))
    {
        fGap += 1;
    }
...
INT64 CSDDateTime::DiffWeek(const CSDDateTime & datetime)
{
    ...
    tm_b = mktime(&t_tm);
    s_oMutex.Unlock();
    return ((INT64)difftime(tm_a, tm_b)) / (60 * 60 * 24 * 7);
}
...
BOOL IsToday(time_t t)
{
    ...
    CSDDateTime oLastTime(t);
    if((oNow.GetYear() == oLastTime.GetYear()) && (oNow.GetMonth() == oLastTime.GetMonth()) && (oNow.GetDay() == oLastTime.GetDay()))
    {
        return TRUE;
    }
...
UINT32 GetDayPassSecond(time_t t)
{
    CSDDateTime iDaySec(t);
    return iDaySec.GetHour() * 3600 + iDaySec.GetMinute() * 60 + iDaySec.GetSecond();
}
