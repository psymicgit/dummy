...
namespace SGDP
{
    ...
    */
    ...
    {
        static const TCHAR digits[] = _SDT("0123456789");
        ...
        *(tp = tmp) = 0;
        while ((ch = *src++) != _SDT('\0'))
        {
            ...
            if ((pch = _SDTStrchr(digits, ch)) != NULL)
            {
    ...
    */
    ...
    {
        struct hostent* host = gethostbyname(_SDTT2A(name));
    ...
    {
        return gethostbyname(_SDTT2A(name));
    }
    ...
    INT32  SDGetLocalIp(unsigned long ip[], INT32 count)
    {
        ...
        INT32 iplocal = SDInetAddr(_SDT("127.0.0.1"));
