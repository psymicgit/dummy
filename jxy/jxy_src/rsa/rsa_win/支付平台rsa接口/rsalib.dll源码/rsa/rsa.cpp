...
char* public_key_decrypt_by_pem(const char* encrypted_text, int encrypted_len, const char* pub_key_pem, int *decrypt_len, char **err_msg)
{
    *err_msg = rsa_err;
    ...
    if(NULL == rsa)
    {
        sprintf_s(rsa_err, "fail: given public key is invalid: %s!\n", pub_key_pem);
        return NULL;
    }
    ...
    if(-1 == len){
        sprintf_s(rsa_err, "failed to decrypt! please recheck the decrypted text and the format of %s\n", pub_key_pem);
        return NULL;
    }
...
char* public_key_decrypt_from_file(const char* encrypted_text, int encrypted_len, const char* pub_key_file, int *decrypt_len, char **err_msg)
{
    *err_msg = rsa_err;
    ...
    if((err = fopen_s(&pub_fp, pub_key_file, "r")) || NULL == pub_fp)
    {
        sprintf_s(rsa_err, "failed to open public key file %s: %s!\n", pub_key_file, strerror_s(err_buf, err));
        return NULL;
    }
    ...
    if(NULL == rsa)
    {
        sprintf_s(rsa_err, "fail: unable to read public key from %s!\n", pub_key_file);
        return NULL;
    }
...
char* private_key_encrypt_pem(const char* plain_text, int plain_len, const char* pri_key_pem, int *encrypt_len, char **err_msg)
{
    ...
    *encrypt_len = 0;
    ...
    if(NULL == rsa)
    {
        sprintf_s(rsa_err, "fail: given private key is invalid!\n");
        return NULL;
    }
    ...
    {
        sprintf_s(rsa_err, "failed to encrypt: length of plain text(= %d) is too long, must be less than rsa_size(=%d) - 41 = %d\n", plain_len, rsa_size, rsa_size - 41);
        return NULL;
    }
    ...
    if(-1 == len){
        sprintf_s(rsa_err, "failed to encrypt! please recheck the plain text and the format of %s\n", pri_key_pem);
        return NULL;
    }
...
char* private_key_encrypt_from_file(const char* plain_text, int plain_len, const char* pri_key_file, int *encrypt_len, char **err_msg)
{
    *err_msg = rsa_err;
    ...
    FILE* priv_fp;
    errno_t err;
    if((err = fopen_s( &priv_fp, pri_key_file, "r" )) || NULL == priv_fp)
    {
        sprintf_s(rsa_err, "failed to open private key file %s: %s!\n", pri_key_file, strerror_s(err_buf, err));
        return NULL;
    }
    ...
    if(NULL == rsa){
        sprintf_s(rsa_err, "fail: unable to read private key from %s!\n", pri_key_file);
        return NULL; 
    }
...
char* private_key_sign_pem(const char* plain_text, int plain_len, const char* pri_key_pem, unsigned int *sign_len, char **err_msg)
{
    ...
    *sign_len = 0;
    ...
    // 采用RSA对sha1摘要进行签名
    int ret = RSA_sign(NID_sha1, (unsigned char*)buf, strlen((char*)buf), encrypt_buf, sign_len, rsa);
    if(ret != 1)
    {
        sprintf_s(rsa_err, "fail: can not sign with given private key!\n");
        return NULL;
    }
...
bool public_key_verify_pem(const char* plain_text, int plain_len, const char* sign_text, int sign_len, const char* pub_key_pem, char **err_msg)
{
    *err_msg = rsa_err;
    ...
    // 采用RSA对sha1摘要进行验签
    int ret = RSA_verify(NID_sha1WithRSA, (unsigned char*)buf, strlen((char*)buf), (unsigned char*)sign_text, sign_len, rsa);
    return (ret == 1);
}