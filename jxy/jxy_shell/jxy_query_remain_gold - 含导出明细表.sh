...
mysql $g_paydbopt $g_pay_db << _EOFMYSQL_
	...
	delete from ${rmb_player} where PlayerID in (select PlayerID from mysql.test_player);
...
mysql $g_paydbopt -vvv $g_pay_db << _EOFMYSQL_
	...
	select sum(Gold) as '${begin_pay_date}到${end_pay_date}充值留存玩家的剩余元宝' from ${roll_back_player_gold};
