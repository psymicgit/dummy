...
g_tables="factionplayer player_bags player_builds player_cdkey player_consume player_ext player_hero_ext player_heros player_instances player_pvp player_recharge player_relation player_tasks private_chat_log send_flower_log"
...
function tie_zone_id()
{
	...
	for ((i=$start_zone_idx;i<=$end_zone_idx;i++))
	{
		zone_id=${g_zone_id_array[i]}

		if [ $i -eq $end_zone_idx ]; then
			zone_id_list="$zone_id_list""$zone_id"
		else
			zone_id_list="$zone_id_list""$zone_id"${joint}
		fi
	}
