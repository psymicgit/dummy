...
BEGIN
	...
	set @g_cur_db = 'ios_app_s017';
	select database() into @g_cur_db;

	if @g_cur_db <> 'ios_app_s017' then
		select concat('请在ios_app_s017数据库下执行本脚本，当前数据库是', @g_cur_db);
		leave lable_1;
	end if;

	# 稍微检测一下两两合区时会不会碰到合表主键冲突（全部检测要 8 * 8 / 2 次，太多了）
	SET @v_is_merge_check_ok=1;
	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s018', '囧18区', 18, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧17区与囧18区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s019', '囧19区', 19, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧17区与囧19区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s020_merge', '囧20区', 20, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧17区与囧20区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s021_merge', '囧21区', 21, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧17区与囧21区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s022_merge', '囧22区', 22, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧17区与囧22区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s023_merge', '囧23区', 23, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧17区与囧23区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s017', '囧17区', 17, 'ios_app_s024_merge', '囧24区', 24, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
	...
	if @v_is_merge_check_ok = 1 then
		...
		# 生成出冲突玩家昵称列表
		call merge_conflict_player_name('ios_app_s017');
		...
		# 生成冲突门派名称列表
		call merge_conflict_faction_name('ios_app_s017');
		...
		call modify_player_name_and_print('ios_app_s017', '囧17区', 'players', '17');
		...
		call modify_faction_name_and_print('ios_app_s017', '囧17区', 'faction', '17');
		...
		# 先分配好合区奖励
		call give_merge_gift(10017, 'ios_app_s017', '囧17区', 'merge_gift_17_18_19_20_21_22_23_24');
...
#正式开始合区，注意，这里只有被合到的区数据会受到改动，如2区、3区合到1区，则只有1区数据会增加------------------------------------------------------------
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s018', '囧18区', 18);
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s019', '囧19区', 19);
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s020_merge', '囧20区', 20);
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s021_merge', '囧21区', 21);
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s022_merge', '囧22区', 22);
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s023_merge', '囧23区', 23);
		call merge_gamedb('ios_app_s017', '囧17区', 17, 'ios_app_s024_merge', '囧24区', 24);
	end if;
end
@@
