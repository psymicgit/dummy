...
BEGIN
	...
	set @g_cur_db = 'ios_app_s033_merge';
	select database() into @g_cur_db;

	if @g_cur_db <> 'ios_app_s033_merge' then
		select concat('请在ios_app_s033_merge数据库下执行本脚本，当前数据库是', @g_cur_db);
		leave lable_1;
	end if;

	# 稍微检测一下两两合区时会不会碰到合表主键冲突（全部检测要 8 * 8 / 2 次，太多了）
	SET @v_is_merge_check_ok=1;
	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s034', '囧34区', 34, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧33区与囧34区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s035', '囧35区', 35, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧33区与囧35区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s036', '囧36区', 36, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧33区与囧36区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s037_merge', '囧37区', 37, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧33区与囧37区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s038_merge', '囧38区', 38, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧33区与囧38区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s039_merge', '囧39区', 39, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧33区与囧39区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s033_merge', '囧33区', 33, 'ios_app_s040_merge', '囧40区', 40, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
	...
	if @v_is_merge_check_ok = 1 then
		...
		# 生成出冲突玩家昵称列表
		call merge_conflict_player_name('ios_app_s033_merge');
		...
		# 生成冲突门派名称列表
		call merge_conflict_faction_name('ios_app_s033_merge');
		...
		call modify_player_name_and_print('ios_app_s033_merge', '囧33区', 'players', '33');
		...
		call modify_faction_name_and_print('ios_app_s033_merge', '囧33区', 'faction', '33');
		...
		# 先分配好合区奖励
		call give_merge_gift(10033, 'ios_app_s033_merge', '囧33区', 'merge_gift_33_34_35_36_37_38_39_40');
...
#正式开始合区，注意，这里只有被合到的区数据会受到改动，如2区、3区合到1区，则只有1区数据会增加------------------------------------------------------------
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s034', '囧34区', 34);
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s035', '囧35区', 35);
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s036', '囧36区', 36);
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s037_merge', '囧37区', 37);
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s038_merge', '囧38区', 38);
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s039_merge', '囧39区', 39);
		call merge_gamedb('ios_app_s033_merge', '囧33区', 33, 'ios_app_s040_merge', '囧40区', 40);
	end if;
end
@@
