...
BEGIN
	...
	set @g_cur_db = 'ios_app_s001_merge';
	select database() into @g_cur_db;

	if @g_cur_db <> 'ios_app_s001_merge' then
		select concat('请在ios_app_s001_merge数据库下执行本脚本，当前数据库是', @g_cur_db);
		leave lable_1;
	end if;

	# 稍微检测一下两两合区时会不会碰到合表主键冲突（全部检测要 8 * 8 / 2 次，太多了）
	SET @v_is_merge_check_ok=1;
	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s002', '囧2区', 2, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧1区与囧2区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s003', '囧3区', 3, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧1区与囧3区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s004', '囧4区', 4, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧1区与囧4区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s005', '囧5区', 5, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧1区与囧5区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s006', '囧6区', 6, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧1区与囧6区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s007_merge', '囧7区', 7, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧1区与囧7区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s001_merge', '囧1区', 1, 'ios_app_s008_merge', '囧8区', 8, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
	...
	if @v_is_merge_check_ok = 1 then
		...
		# 生成出冲突玩家昵称列表
		call merge_conflict_player_name('ios_app_s001_merge');
		...
		# 生成冲突门派名称列表
		call merge_conflict_faction_name('ios_app_s001_merge');
		...
		call modify_player_name_and_print('ios_app_s001_merge', '囧1区', 'players', '1');
		...
		call modify_faction_name_and_print('ios_app_s001_merge', '囧1区', 'faction', '1');
		...
		# 先分配好合区奖励
		call give_merge_gift(10001, 'ios_app_s001_merge', '囧1区', 'merge_gift_1_2_3_4_5_6_7_8');
...
#正式开始合区，注意，这里只有被合到的区数据会受到改动，如2区、3区合到1区，则只有1区数据会增加------------------------------------------------------------
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s002', '囧2区', 2);
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s003', '囧3区', 3);
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s004', '囧4区', 4);
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s005', '囧5区', 5);
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s006', '囧6区', 6);
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s007_merge', '囧7区', 7);
		call merge_gamedb('ios_app_s001_merge', '囧1区', 1, 'ios_app_s008_merge', '囧8区', 8);
	end if;
end
@@
