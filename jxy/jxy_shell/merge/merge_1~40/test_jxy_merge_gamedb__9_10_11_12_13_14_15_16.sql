...
BEGIN
	...
	set @g_cur_db = 'ios_app_s009_merge';
	select database() into @g_cur_db;

	if @g_cur_db <> 'ios_app_s009_merge' then
		select concat('请在ios_app_s009_merge数据库下执行本脚本，当前数据库是', @g_cur_db);
		leave lable_1;
	end if;

	# 稍微检测一下两两合区时会不会碰到合表主键冲突（全部检测要 8 * 8 / 2 次，太多了）
	SET @v_is_merge_check_ok=1;
	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s010', '囧10区', 10, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧9区与囧10区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s011', '囧11区', 11, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧9区与囧11区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s012', '囧12区', 12, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧9区与囧12区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s013', '囧13区', 13, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧9区与囧13区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s014', '囧14区', 14, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧9区与囧14区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s015', '囧15区', 15, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧9区与囧15区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s009_merge', '囧9区', 9, 'ios_app_s016', '囧16区', 16, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
	...
	if @v_is_merge_check_ok = 1 then
		...
		# 生成出冲突玩家昵称列表
		call merge_conflict_player_name('ios_app_s009_merge');
		...
		# 生成冲突门派名称列表
		call merge_conflict_faction_name('ios_app_s009_merge');
		...
		call modify_player_name_and_print('ios_app_s009_merge', '囧9区', 'players', '9');
		...
		call modify_faction_name_and_print('ios_app_s009_merge', '囧9区', 'faction', '9');
		...
		# 先分配好合区奖励
		call give_merge_gift(10009, 'ios_app_s009_merge', '囧9区', 'merge_gift_9_10_11_12_13_14_15_16');
...
#正式开始合区，注意，这里只有被合到的区数据会受到改动，如2区、3区合到1区，则只有1区数据会增加------------------------------------------------------------
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s010', '囧10区', 10);
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s011', '囧11区', 11);
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s012', '囧12区', 12);
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s013', '囧13区', 13);
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s014', '囧14区', 14);
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s015', '囧15区', 15);
		call merge_gamedb('ios_app_s009_merge', '囧9区', 9, 'ios_app_s016', '囧16区', 16);
	end if;
end
@@
