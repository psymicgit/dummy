...
BEGIN
	...
	set @g_cur_db = 'ios_app_s025';
	select database() into @g_cur_db;

	if @g_cur_db <> 'ios_app_s025' then
		select concat('请在ios_app_s025数据库下执行本脚本，当前数据库是', @g_cur_db);
		leave lable_1;
	end if;

	# 稍微检测一下两两合区时会不会碰到合表主键冲突（全部检测要 8 * 8 / 2 次，太多了）
	SET @v_is_merge_check_ok=1;
	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s026', '囧26区', 26, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧25区与囧26区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s027', '囧27区', 27, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧25区与囧27区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s028', '囧28区', 28, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧25区与囧28区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s029', '囧29区', 29, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧25区与囧29区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s030', '囧30区', 30, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧25区与囧30区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s031', '囧31区', 31, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
		select @v_is_merge_check_ok as '检测到囧25区与囧31区的合区将发生冲突';
	end if;

	call merge_gamedb_check('ios_app_s025', '囧25区', 25, 'ios_app_s032', '囧32区', 32, @v_is_merge_check_ok);
	if @v_is_merge_check_ok = 0 then
	...
	if @v_is_merge_check_ok = 1 then
		...
		# 生成出冲突玩家昵称列表
		call merge_conflict_player_name('ios_app_s025');
		...
		# 生成冲突门派名称列表
		call merge_conflict_faction_name('ios_app_s025');
		...
		call modify_player_name_and_print('ios_app_s025', '囧25区', 'players', '25');
		...
		call modify_faction_name_and_print('ios_app_s025', '囧25区', 'faction', '25');
		...
		# 先分配好合区奖励
		call give_merge_gift(10025, 'ios_app_s025', '囧25区', 'merge_gift_25_26_27_28_29_30_31_32');
...
#正式开始合区，注意，这里只有被合到的区数据会受到改动，如2区、3区合到1区，则只有1区数据会增加------------------------------------------------------------
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s026', '囧26区', 26);
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s027', '囧27区', 27);
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s028', '囧28区', 28);
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s029', '囧29区', 29);
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s030', '囧30区', 30);
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s031', '囧31区', 31);
		call merge_gamedb('ios_app_s025', '囧25区', 25, 'ios_app_s032', '囧32区', 32);
	end if;
end
@@
