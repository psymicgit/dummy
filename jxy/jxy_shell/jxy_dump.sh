...
echo_blue "start dump databases $dbs @ [${mysqlhost}:${mysqlport}] to $dump_dir"
...
_EOFMYSQL_
	...
	mysqldump $mysqlopt $v --add-locks --add-drop-table --single-transaction --compress --extended-insert --opt --default-character-set=utf8 --events --hex-blob --skip-comments  --triggers --routines $extra_opt $db > "$dump_dir"/$db.sql
	# --skip-quick --add-drop-database --databases 
