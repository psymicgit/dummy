...
function create_player_dec_gold()
{
	# 服务器列表文件
	local zone_list=$1
...
mysql $gamedbopt $gamedb << _EOFMYSQL_
		...
		insert ignore into ${rmb_player_dec_gold} select PlayerID, 0, 0 from players where PlayerID > 1010 and Pay > 0;

		update ${rmb_player_dec_gold} r,(select c.PlayerID, sum(c.AddGold) as sumaddgold from player_consume c,players p where c.PlayerID = p.PlayerID and p.Pay > 0 and c.AddGold > 0 and c.CreateTime >= '${pay_date_beg}' and c.CreateTime < '${pay_date_end}' group by c.PlayerID) g set r.AddGold = g.sumaddgold where r.PlayerID = g.PlayerID;

		update ${rmb_player_dec_gold} r,(select c.PlayerID, sum(c.DecGold) as sumdecgold from player_consume c,players p where c.PlayerID = p.PlayerID and p.Pay > 0 and c.DecGold > 0 and c.CreateTime >= '${pay_date_beg}' and c.CreateTime < '${pay_date_end}' group by c.PlayerID) g set r.DecGold = g.sumdecgold where r.PlayerID = g.PlayerID;
_EOFMYSQL_
...
function query_rmb_player_dec_gold()
{
	local zone_list=$1
	local pay_date_beg=$2
	local pay_date_end=$3

	create_rmb_player ${pay_date_beg} ${pay_date_end}

	create_player_dec_gold ${zone_list} ${pay_date_beg} ${pay_date_end}
...
function query_ios()
{
	...
	query_rmb_player_dec_gold "${ios_zone_list}" "2014-01-01" "2014-04-01"
	query_rmb_player_dec_gold "${ios_zone_list}" "2014-02-01" "2014-05-01"
	query_rmb_player_dec_gold "${ios_zone_list}" "2014-03-01" "2014-06-01"
	query_rmb_player_dec_gold "${ios_zone_list}" "2014-04-01" "2014-07-01"

	query_rmb_player_dec_gold "${ios_zone_list}" "2014-04-01" "2014-07-01"
	query_rmb_player_dec_gold "${ios_zone_list}" "2014-05-01" "2014-07-01"
	query_rmb_player_dec_gold "${ios_zone_list}" "2014-06-01" "2014-07-01"
}
...
function query_and()
{
	...
	query_rmb_player_dec_gold "${and_zone_list}" "2014-01-01" "2014-04-01"
	query_rmb_player_dec_gold "${and_zone_list}" "2014-02-01" "2014-05-01"
	query_rmb_player_dec_gold "${and_zone_list}" "2014-03-01" "2014-06-01"
	query_rmb_player_dec_gold "${and_zone_list}" "2014-04-01" "2014-07-01"

	query_rmb_player_dec_gold "${and_zone_list}" "2014-04-01" "2014-07-01"
	query_rmb_player_dec_gold "${and_zone_list}" "2014-05-01" "2014-07-01"
	query_rmb_player_dec_gold "${and_zone_list}" "2014-06-01" "2014-07-01"
}
